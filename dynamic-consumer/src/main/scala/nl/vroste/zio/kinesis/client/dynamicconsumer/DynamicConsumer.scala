...
object DynamicConsumer {
  ...
  trait CheckpointerInternal extends Checkpointer {
      ...
      for {
        ...
        override def checkpoint: ZIO[Blocking, Throwable, Unit] =
          latestStaged.get.flatMap {
            case Some(record) =>
              for {
                _ <- logger.info(s"about to checkpoint: shardId=${record.shardId} partitionKey=${record.partitionKey}")
