...
object DynamicConsumerTest extends DefaultRunnableSpec {
  ...
  private val env: ZLayer[Any, Throwable, Client with AdminClient with DynamicConsumer with Clock] =
    (LocalStackServices.localHttpClient >>> LocalStackServices.kinesisAsyncClientLayer >>> (Client.live ++ AdminClient.live ++ (LocalStackServices.localStackAwsLayer >>> DynamicConsumer.live))) ++ Clock.live
